...
def with_timeout(timeout_seconds: int):
    """Decorator to add timeout protection to async endpoints."""
    def decorator(func):
        @wraps(func)
        async def wrapper(*args, **kwargs):
            try:
                ...
                logger.error(f"[TIMEOUT] {func.__name__} exceeded {timeout_seconds}s timeout")
                raise HTTPException(504, f"Request timed out after {timeout_seconds}s. Please try again.")
        return wrapper
    return decorator
